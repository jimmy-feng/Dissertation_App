...
# Person 3. Single white man with a GED who works night-time shifts as a janitor for a local bank while attending Tennessee College of Applied Technology during the day to become an electrician. Originally from south Alabama and relocated here because of a lack of job opportunities. Rents a room in South Knoxville and commutes to work (Bank of America in downtown) by foot to save money. Relies on public transit to get to college. ($28,685 annual gross salary, $24,808 take-home salary).
# Person 6. Divorced mother of two and military veteran who finished her Bachelor’s in history and education at UTK but is struggling to find full-time work; every day she does not know if she’ll be teaching the following day and is always ready to take any substitute roles at schools in need. So her schedule is restricted on the weekdays. She earns $15,184 a year and receives $400 in SNAP benefits each month. Lives with her elderly mother in a small home in South Knoxville. Car was totaled in a recent car crash, so she currently relies on public transit to get around the county.
# Person 22. Siemens machinist who works weekends, is a single Black mother with two kids, mid-30’s, contemplating going back to university for engineering degree and also having groceries delivered online. Lives in east Knoxville.
# Person 35. 21 year-old Black male college student living at home in Hardin Valley. Works multiple jobs: part-time associate at REI and cook at Waffle House -- while taking on full course load at Pellissippi State Community College in Hardin Valley.
# Person 42. 38 year-old black Adjunct professor who teaches four classes at UTK. Lives with wife and one baby in an apartment in South Knoxville. Drops off his baby at the day care before work every morning. Doesn’t have much time to carry out errands; prefers quick visits, curbside pickup or deliveries.
# Person 44. 56 year-old Hispanic female hotel clerk working in Strawberry Plains and lives in Fountain City. Prefers driving not too far and not walking because she perceives the neighborhood as dangerous. Is separated from husband and lives in an apartment with adult daughter and boyfriend. Works early shift on weekdays. Drops off her daughter at her workplace at a Dollar General in West Hills on weekends before her afternoon shift.

# TO-DO:
# 5. Design
# Cytoscape design: https://js.cytoscape.org/#style/mappers
# Font: https://stackoverflow.com/questions/69258350/difficulty-getting-custom-google-font-working-for-plotly-dash-app
# 6. Upload to Heroku: https://github.com/danny-baker/dash-heroku 
# https://towardsdatascience.com/beginners-guide-to-building-a-multi-page-dashboard-using-dash-5d06dbfc7599
# https://dash.plotly.com/deployment
# 6a. or in browser: https://github.com/ibdafna/webdash
# Inspo for Dash-Leaflet: https://dash-leaflet.herokuapp.com/

# python "C://Users//jfeng/OneDrive - University of Tennessee//Research//Dissertation_Space_Time/Code/Visualize_POI_Dash.py"
...
def network_data(od_df, individuals_df, stores_df, stores_online_df):
    ...
    for i, r in od_condensed.iterrows():
        ...
        if str(r['person_id']) not in nodes:
            ...
            # if r['person_id'] in individuals_df['person_id']:
            #     if len(individuals_df[individuals_df['person_id'] == r['person_id']]['PhysicalOpp']) > 0:
            #         p_opp = individuals_df[individuals_df['person_id'] == r['person_id']]['PhysicalOpp']
            #     else:
            #         p_opp = 0
            #     if len(individuals_df[individuals_df['person_id'] == r['person_id']]['VirtualOpp']) > 0:
            #         p_opp = individuals_df[individuals_df['person_id'] == r['person_id']]['VirtualOpp']
            #     else:
            #         p_opp = 0
            #     store_to_person = p_opp + individuals_df[individuals_df['person_id'] == r['person_id']]['VirtualOpp']
            # else:
            #     store_to_person = 0
        ...
        # if r['person_id'] not in nodes:
        #     nodes.add(r['person_id'])
        #     if isinstance(r['person_id'], int):
        #         store_to_person = individuals_df[individuals_df['person_id'] == r['person_id']]['PhysicalOpp'] + individuals_df[individuals_df['person_id'] == r['person_id']]['VirtualOpp']
        #         node_size = int(np.sqrt(1 + store_to_person) * 10)
        #         id_label = 'Person ' + str(r['person_id'])
        #         cy_nodes.append({'data': {'id': r['person_id'], 'label': id_label, 'node_size': node_size}, 'classes': 'person'}) # , 'node_size': ))
        #     # else: 
        #     #     id_label = r['person_id']
        #     #     cy_nodes.append({'data': {'id': r['store_PlusCode'], 'label': stores[stores['PlusCode'] == id_label]['Name'].values[0] + ' - ' + r['store_PlusCode']}, 'classes': 'store'})
        # if r['store_PlusCode'] not in nodes:
        #     nodes.add(str(r['store_PlusCode']))
        #     if isinstance(r['store_PlusCode'], int):
        #         store_to_person = individuals_df[individuals_df['person_id'] == r['person_id']]['PhysicalOpp'] + individuals_df[individuals_df['person_id'] == r['person_id']]['VirtualOpp']
        #         node_size = int(np.sqrt(1 + store_to_person) * 10)
        #         id_label = 'Person ' + str(r['store_PlusCode'])
        #         cy_nodes.append({'data': {'id': r['person_id'], 'label': id_label, 'node_size': node_size}, 'classes': 'person'})
        #     else:
        #         people_to_store = od_condensed[od_condensed['store_PlusCode'] == r['store_PlusCode']]['person_id'].nunique()
        #         node_size = int(np.sqrt(1 + people_to_store) * 10)
        #         id_label = r['store_PlusCode']
        #         cy_nodes.append({'data': {'id': r['store_PlusCode'], 'label': stores[stores['PlusCode'] == id_label]['Name'].values[0] + ' - ' + r['store_PlusCode'], 'node_size': node_size}, 'classes': 'store'})
    ...
    for p in individuals['person_id'].unique():
        if not any(n['data']['id'] == str(p) for n in cy_nodes):
            ...
            #cy_edges.append({'data': {'source': str(p), 'target': str(p)}, 'classes': 'self'})
    ...
    for s in stores['PlusCode'].unique():
        if not any(n['data']['id'] == str(s) for n in cy_nodes):
            ...
            #cy_edges.append({'data': {'source': s, 'target': s}, 'classes': 'self'})      
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            html.H3('Multidimensional Multispace GIS for Access',
...
def update_table_map(page_current_ind, page_size_ind, sort_by_ind, filter_query_ind, page_current, page_size, sort_by, filter_query, map_2d_dropdown_value, restyleData3d, clickData3d, startDate, endDate):
    ...
    # Filtered OD entries
#     od_st = od[(od['free_time_start'] >= startDate) & (od['free_time_end'] <= endDate) & (od['can_visit_spacetime']) & (od['person_id'].isin(individuals_filter['person_id'].unique()))]
#     # OD rows where people can physically reach opportunities in space-time
#     od_visit_spacetime = od_st['can_visit_spacetime']
#     # OD rows for people who are digitally literate
#     od_digitallit = od_st['person_id'].isin(individuals_filter[individuals_filter['digitallit'] == 'yes']['person_id'].values)
#     # OD rows where the store has online shopping option
#     od_shop_online = od_st['store_PlusCode'].isin(stores_filter[stores_filter['Shop_Online'] == True]['PlusCode'].values)    

    
#     hybrid_pluscode = od_st[od_visit_spacetime & od_digitallit & od_shop_online]['store_PlusCode'].drop_duplicates().values
#     physical_pluscode = od_st[~od_st['store_PlusCode'].isin(hybrid_pluscode) & od_visit_spacetime]['store_PlusCode'].drop_duplicates().values
    # virtual_pluscode = get_online_stores(stores_filter[stores_filter['PlusCode'].isin(od[~(od['store_PlusCode'].isin(hybrid_pluscode)) & ~(od['store_PlusCode'].isin(physical_pluscode))]['store_PlusCode'].drop_duplicates().values) & stores_filter['Shop_Online']])['PlusCode'].values
    # stores_filter_online = get_online_stores(stores_filter[stores_filter['PlusCode'].isin(od_st[~(od_st['store_PlusCode'].isin(hybrid_pluscode)) & ~(od_st['store_PlusCode'].isin(physical_pluscode))]['store_PlusCode'].drop_duplicates().values) & stores_filter['Shop_Online']])

    
    # physical_opp_total = len(physical_pluscode)
    # virtual_opp_total = len(virtual_pluscode)
    # hybrid_opp_total = len(hybrid_pluscode)    
...
#     opps_list = []
#     for indiv_i, indiv_r in individuals_filter.iterrows():
#         physical_opps_person = od[(od['person_id'] == indiv_r['person_id']) & (od['free_time_start'] >= startDate) & (od['free_time_end'] <= endDate) & (od['can_visit_spacetime'])]['store_PlusCode'].nunique()
#         if indiv_r['digitallit'] == 'yes':
#             opps_list.append({'person_id': indiv_r['person_id'], 'PhysicalOpp': physical_opps_person, 'VirtualOpp': virtual_opp_total})
#         else:
#             opps_list.append({'person_id': indiv_r['person_id'], 'PhysicalOpp': physical_opps_person, 'VirtualOpp': 0})

#     opps_df = pd.DataFrame(opps_list, columns=['person_id', 'PhysicalOpp', 'VirtualOpp'])
#     individuals_filter = pd.merge(individuals_filter.drop(columns=['PhysicalOpp', 'VirtualOpp']), opps_df, left_on='person_id', right_on='person_id', how='left')
